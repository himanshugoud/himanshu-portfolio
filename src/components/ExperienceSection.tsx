...
const timeline=[
 ...
 {year:"'21",title:"Pragati Vidya Peeth Morar",detail:"High School, CBSE · Gwalior, India",tag:"High School",color:"#ffc800",note:"Where it started"},
];
export default function ExperienceSection(){return <section className="experience content-col">
 ...
 <div className="timeline"><div className="timeline-line"/>{timeline.map((e,i)=><motion.div key={e.title} className="timeline-item" initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.08}}><div className="timeline-year-row"><div className="timeline-year" style={{color:e.color}}>{e.year}</div>{e.note && <span className="timeline-note">{e.note}</span>}</div><div className="timeline-dot" style={{borderColor:e.color}}/><div className="timeline-content"><div className="timeline-top"><h3>{e.title}</h3><span>{e.tag}</span></div><p>{e.detail}</p></div></motion.div>)}</div>
 </section>}
